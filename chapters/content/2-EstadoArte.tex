...
A arquitetura cliente-servidor \ref{fig:client-server} é um modelo amplamente utilizado na 
...
O Apache Storm, um sistema de \textit{streaming} distribuído, foi reconhecido como um sistema de 
...
natureza distribuída dos sistemas de processamento do fluxos de dados.
...
de implantação de \textit{software} para sistemas complexos \cite{canary2022}. É também importante 
destacar a importância das \textit{pipelines} automatizadas de implantação no contexto de \ac{CI} e 
\ac{CD}, onde o sucesso da adoção das alterações nas empresas depende fortemente das 
...
ambiente "azul" para o ambiente "verde" \ref{fig:blue-green-after}, permitindo atualizações 
sem indisponibilidade e simplificando o processo de reversão das alterações, se necessário \cite{canary2022}.
